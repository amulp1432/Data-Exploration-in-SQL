...
order by 1,2
--Data has shown us that death_percentage has start increasing from April 2020.

--Looking at total_cases vs Population
--It will show us that what percentage of people has been affected by the covid
Select location, date,population,total_cases,round((total_cases/population)*100,4) as Affected_rate_by_the_population
from portfolioproject..Covid_deaths$
order by 1,2
--Looking   at the country with highest affected rate by covids
Select location,population,max(total_cases) as Highest_infection_count,round(max(total_cases/population)*100,4) as Affected_rate_by_the_population
from portfolioproject..Covid_deaths$
...
select date,total_deaths,total_cases,round((total_deaths/total_cases)*100,4) as death_percentage
from portfolioproject..Covid_deaths$
where continent is not null
...
sum(cast(vac.new_vaccinations as bigint )) over (partition by dea.location order by dea.location) as total_vaccinations
...
Select *, round((RollingPeopleVaccinated/Population)*100,5) as vaccination_percentage
From PopvsVac
